slide <- function(data, CTMM = NULL, window, dt.min = 0, variable = "area", 
  ...
  for (window_start_index in seq_along(window_starts)) {
    ...
    # Initialize list to store the covariate estimates from each dataset
    dataset_mean_covariate <- vector("list", length(data))
    ...
    for (i in seq_along(data)) {  # Iterate over each dataset
      ...
      if (variable == "position" || variable == "velocity" || variable == "diffusion") {
        window_estimate[[i]] <- FIT
      }
      # Calculate mean covariate for each dataset and store in dataset_mean_covariate
      if (!is.null(covariate)) {
        dataset_mean_covariate <- mean(subset_data[[covariate]], na.nm = TRUE)
      }
    }
    ...
    if (individual_df) {
      ...
      # Ensure proper handling of summary results
    ...
    #Calculate mean of covariates from each dataset
    covariate <- mean(dataset_mean_covariate)
